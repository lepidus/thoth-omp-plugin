...
class ThothWorkRelationService
{
    ...
    private function getDesiredChapters(array $chapters, string $thothImprintId): array
    {
        $desiredChapters = [];
        foreach ($chapters as $chapter) {
    ...
    private function findMatchingRelationKey(
        ...
        $key = $this->findUniqueMatch(
            $relations,
            fn (array $relation): bool => $desiredChapter['doi'] !== null
        ...
        if ($landingPage !== null && ($desiredLandingPageCounts[$landingPage] ?? 0) === 1) {
            $key = $this->findUniqueMatch(
                $relations,
                fn (array $relation): bool => $this->normalizeUrl(
        ...
        if ($title !== '' && ($desiredTitleCounts[$title] ?? 0) === 1) {
            $key = $this->findUniqueMatch(
                $relations,
                fn (array $relation): bool => $this->normalizeTitle(
        ...
        return $this->findUniqueMatch(
            $relations,
            fn (array $relation): bool => (int) ($relation['relationOrdinal'] ?? 0)
                === $desiredChapter['ordinal'],
            'relation ordinal'
        );
    }

    private function findUniqueMatch(array $relations, callable $matches, string $identity): ?int
    {
        $matchingKeys = [];
    ...
    private function assertUniqueDesiredDois(array $desiredChapters): void
    {
    ...
    private function countDesiredValues(array $desiredChapters, string $key): array
    {
        $counts = [];
        foreach ($desiredChapters as $desiredChapter) {
    ...
    private function getTemporaryOrdinal(array $existingRelations, array $desiredChapters): int
    {
        $ordinals = array_map(
            fn (array $relation): int => (int) ($relation['relationOrdinal'] ?? 0),
            $existingRelations
        );
        $ordinals = array_merge($ordinals, array_column($desiredChapters, 'ordinal'));

        return max([0, ...$ordinals]) + count($existingRelations) + 1;
    }

    private function editRelationOrdinal(array $relation, int $ordinal): void
    {
