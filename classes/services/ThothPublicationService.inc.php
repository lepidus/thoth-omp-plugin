...
class ThothPublicationService
{
    ...
    public function update(
        ...
        $skipPublicationDeletions = $workStatus === WorkStatus::ACTIVE && !empty($remainingPublications);
        if (!$skipPublicationDeletions) {
            foreach ($remainingPublications as $remainingPublication) {
                $this->repository->delete($remainingPublication['publicationId']);
            }
        }

        return $skipPublicationDeletions;
    }

    public function registerByChapter($chapter)
    {
        list($publicationFormats, $submissionFilesByPublicationFormat) =
            $this->getChapterPublicationData($chapter);
    ...
    private function getChapterPublicationData($chapter)
    {
