...
class ThothEndpoint implements HasAuthorizationPolicy
{
    ...
    public function getFeatureVideoForm(IlluminateRequest $illuminateRequest): JsonResponse
    {
        ...
        try {
            $workRepository = \PKP\core\PKPContainer::getInstance()->make('workRepository');
            ...
            $form = new FeatureVideoForm(
                $featureVideoUrl,
                $temporaryFilesUrl,
                \PKP\core\PKPContainer::getInstance()->make('meService')->hasCdnWritePermission(),
                (bool) $existingVideo
            );
    ...
    public function uploadFeatureVideo(IlluminateRequest $illuminateRequest): JsonResponse
    {
        ...
        try {
            if (!\PKP\core\PKPContainer::getInstance()->make('meService')->hasCdnWritePermission()) {
                return response()->json(
