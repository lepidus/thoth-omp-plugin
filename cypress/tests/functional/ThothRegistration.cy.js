	...
	it('registers a published monograph with complete metadata using the Register button', function () {
		...
		readRegisteredWork(this.book.key).then((work) => {
			...
			expect(work.subjects).to.have.deep.members([
				...
				{subjectType: 'CUSTOM', subjectCode: 'open-research', subjectOrdinal: 5},
			...
			// Verify contributor identity, translated biographies and the ROR affiliation.
			...
			expect(contribution.affiliations).to.deep.eq([{affiliationOrdinal: 1, institution: {ror: metadata.ror}}]);
