...
describe('Thoth book registration', function () {
	...
	it('registers a published monograph with complete metadata using the Register button', function () {
		...
		cy.route('POST', `**/api/v1/submissions/${this.book.submissionId}/register`).as('register');
		...
		cy.wait('@register').then(({status, responseBody}) => {
			expect(status).to.eq(200);
			expect(responseBody.thothWorkId).to.match(/^[a-f0-9-]{36}$/);
		});
		...
		readRegisteredWork(this.book.key).then((work) => {
			...
			expect(work.titles).to.deep.include({localeCode: 'EN_US', title: `The ${this.book.title}`,
			...
			expect(work.abstracts).to.have.deep.members([
				{localeCode: 'EN_US', content: '<p>A study of <bold>open science</bold>.</p>', canonical: true, abstractType: 'LONG'},
			...
			expect(contribution.biographies).to.have.deep.members([
				{localeCode: 'EN_US', content: '<p>Researcher in <bold>publishing</bold>.</p>', canonical: true},
			...
			expect(chapter.titles).to.have.deep.members([
				{localeCode: 'EN_US', title: 'Opening knowledge', subtitle: 'An introduction',
			...
			expect(chapter.abstracts).to.have.deep.members([
				{localeCode: 'EN_US', content: '<p>Chapter overview.</p>', canonical: true, abstractType: 'LONG'},
			...
			expect(chapter.contributions[0]).to.deep.eq({...contribution, mainContribution: true});
		});
		cy.reload();
		cy.get('.pkpPublication__thoth').contains('span', /^\s*Active\s*$/).scrollIntoView();
		cy.get('.pkpPublication__thoth').contains('span', /^\s*Active\s*$/).should('be.visible');
