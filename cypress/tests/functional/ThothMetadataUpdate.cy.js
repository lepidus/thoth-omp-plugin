...
describe('Thoth metadata synchronization', function () {
	...
	it('updates the existing Thoth work with current OMP metadata', function () {
		...
		cy.intercept('PUT', `**/_submissions/${this.book.submissionId}/publications/${this.book.publicationId}/synchronize`)
			.as('synchronize');
		cy.intercept('POST', '**/notification/fetchNotification').as('notification');
		cy.contains('span', 'Thoth Status:').parent()
			.contains('button', /^\s*Update Metadata\s*$/).scrollIntoView().click();
		cy.wait('@synchronize').its('response.statusCode').should('eq', 200);
		cy.wait('@notification').its('response.statusCode').should('eq', 200);
