...
function seedCompleteMetadata(array $fixture, array $credentials): array
{
    ...
    Repo::controlledVocab()->insertBySymbolic('submissionKeyword', [
        'en' => ['open science', 'university presses'],
    ], \APP\core\Application::ASSOC_TYPE_PUBLICATION, $publicationId);
    Repo::controlledVocab()->insertBySymbolic('submissionSubject', [
        'en' => [
            ['name' => 'Education', 'source' => 'BISAC', 'identifier' => 'EDU000000'],
            ['name' => 'Education', 'source' => 'THEMA', 'identifier' => 'JN'],
            ['name' => 'Education', 'source' => 'BIC', 'identifier' => 'JN'],
            ['name' => 'Library science', 'source' => 'LCC', 'identifier' => 'Z665'],
            ['name' => 'Open research', 'source' => 'cypress-vocabulary', 'identifier' => 'open-research'],
        ],
    ], \APP\core\Application::ASSOC_TYPE_PUBLICATION, $publicationId);

    // Match the named Author group in this press; role IDs are shared with other contributor roles.
    $authorGroup = \PKP\userGroup\UserGroup::query()->where('context_id', 1)
        ->get()->filter(fn ($group) => $group->nameLocaleKey === 'default.groups.name.author')->sole();
    ...
    $authorId = Repo::author()->add(Repo::author()->newDataObject([
        'publicationId' => $publicationId, 'userGroupId' => $authorGroup->id,
    ...
    // The institution is a remote precondition; the plugin must create its affiliation itself.
    $ror = 'https://ror.org/02y3ad647';
    $institutions = thothFixtureGraphql(
        $credentials,
        'query($ror: String!) { institutions(filter: $ror) { institutionId ror } }',
        ['ror' => $ror]
    )['institutions'];
    if (!$institutions) {
        thothFixtureGraphql(
            $credentials,
            'mutation($data: NewInstitution!) { createInstitution(data: $data) { institutionId } }',
            ['data' => ['institutionName' => 'Cypress Test Institution', 'ror' => $ror]]
        );
    }
    Repo::affiliation()->add(Repo::affiliation()->newDataObject([
        'authorId' => $authorId, 'ror' => $ror, 'name' => ['en' => 'Cypress Test Institution'],
    ]));
    ...
    return ['metadata' => ['doi' => 'https://doi.org/' . $doi, 'chapterDoi' => 'https://doi.org/' . $chapterDoi,
        ...
        'coverName' => $coverName, 'authorFamilyName' => 'Author ' . $key, 'ror' => $ror, 'orcid' => $orcid]];
}
