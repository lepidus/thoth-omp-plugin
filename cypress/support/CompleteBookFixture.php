...
function seedCompleteMetadata(array $fixture, array $credentials): array
{
    ...
    $publicFiles = new PublicFileManager();
    $publicFiles->writeContextFile(1, $coverName, base64_decode(
        'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+aX1sAAAAASUVORK5CYII='
    ));
    $publication = Services::get('publication')->get($publicationId);
    $publication->setData('prefix', ['en_US' => 'The', 'pt_BR' => 'A']);
    $publication->setData('title', ['en_US' => $fixture['title'], 'pt_BR' => 'Ciência aberta ' . $key]);
    $publication->setData('subtitle', ['en_US' => 'Practices and perspectives', 'pt_BR' => 'Práticas e perspectivas']);
    $publication->setData('abstract', [
        'en_US' => '<p>A study of <strong>open science</strong>.</p>',
        'pt_BR' => '<p>Um estudo sobre <em>ciência aberta</em>.</p>',
    ]);
    $publication->setData('licenseUrl', 'https://creativecommons.org/licenses/by/4.0/');
    $publication->setData('copyrightHolder', ['en_US' => 'Cypress Authors']);
    $publication->setData('copyrightYear', 2020);
    $publication->setData('coverImage', ['en_US' => [
    ...
    DAORegistry::getDAO('SubmissionKeywordDAO')->insertKeywords([
        'en_US' => ['open science', 'university presses'],
    ], $publicationId);
    ...
    DAORegistry::getDAO('SubmissionSubjectDAO')->insertSubjects([
        'en_US' => ['BISAC:EDU000000', 'THEMA:JN', 'BIC:JN', 'LCC:Z665', 'open research'],
    ], $publicationId);
    ...
    $authorGroups = DB::table('user_groups as groups')
        ...
        ->where('settings.locale', 'en_US')->where('settings.setting_value', 'Author')
    ...
    $author->setAllData([
        ...
        'locale' => 'en_US', 'givenName' => ['en_US' => 'Cypress'], 'familyName' => ['en_US' => 'Author ' . $key],
        'email' => 'author-' . $key . '@example.org', 'includeInBrowse' => true,
        'url' => 'https://example.org/authors/' . $key,
        'orcid' => $orcid,
        'biography' => ['en_US' => '<p>Researcher in <strong>publishing</strong>.</p>',
    ...
    foreach (['PDF' => 'DA', 'EPUB' => 'DA', 'Paperback' => 'BC'] as $name => $entryKey) {
        ...
        $format->setAllData([
            'publicationId' => $publicationId, 'name' => ['en_US' => $name], 'entryKey' => $entryKey,
    ...
    $chapter->setAllData([
        ...
        'title' => ['en_US' => 'Opening knowledge', 'pt_BR' => 'Abrindo o conhecimento'],
        'subtitle' => ['en_US' => 'An introduction', 'pt_BR' => 'Uma introdução'],
        'abstract' => ['en_US' => '<p>Chapter overview.</p>', 'pt_BR' => '<p>Visão geral do capítulo.</p>'],
        'pub-id::doi' => $chapterDoi,
    ]);
    $chapterId = $chapterDao->insertChapter($chapter);
    DAORegistry::getDAO('ChapterAuthorDAO')->insertChapterAuthor($authorId, $chapterId, true, 0);
