...
try {
    ...
    if ($command === 'configure') {
        ...
        $settings->updateSetting(1, 'doipubidplugin', 'enabled', true, 'bool');
        ...
        // DOI is provided by a core plugin in OMP 3.3.
        PluginRegistry::loadCategory('pubIds', true, 1);
        $published = in_array($command, ['create', 'create-complete'], true);
        $status = $published ? STATUS_PUBLISHED : STATUS_QUEUED;
        ...
        $submission->setAllData([
            'contextId' => 1, 'locale' => 'en_US', 'status' => $status,
            'stageId' => WORKFLOW_STAGE_ID_PRODUCTION, 'submissionProgress' => 0,
            'workType' => WORK_TYPE_AUTHORED_WORK,
        ]);
        $publication = DAORegistry::getDAO('PublicationDAO')->newDataObject();
        $publication->setAllData([
            'locale' => 'en_US', 'title' => ['en_US' => $title], 'abstract' => ['en_US' => $book['abstract']],
            'urlPath' => 'thoth-cypress-' . $key,
            'status' => $status, 'datePublished' => $published ? '2020-01-01' : null,
        ]);
        $submission->stampLastActivity();
        $submission->stampModified();
        $submission->setData('dateSubmitted', date('Y-m-d H:i:s'));
        $id = DAORegistry::getDAO('SubmissionDAO')->insertObject($submission);
        $publication->setData('submissionId', $id);
