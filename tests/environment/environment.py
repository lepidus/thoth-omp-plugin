...
def main():
    ...
    parser.add_argument("--dataset", type=Path, help="OMP stable-3_3_0 MySQL dataset (database.sql, files/, public/)")
    ...
    if args.spec and (Path(args.spec).name != args.spec or not args.spec.endswith(".spec.js")
                      or not (ROOT.parent.parent / "cypress/tests/functional" / args.spec).is_file()):
        parser.error("--spec must name an existing .spec.js file in cypress/tests/functional")
    ...
    with open(STATE / "lock", "w") as lock:
        ...
        if args.command == "down":
            ...
            print("OMP and Thoth prepared. Use open --apply or run [--spec NAME.spec.js] --apply.")
        else:
...
def check_connection(run):
    try:
        run("exec", "-T", "cypress", "curl", "-fsS", "-o", "/dev/null",
            "http://127.0.0.1:8001/index.php/publicknowledge/login")
...
def write_interactive_config(config):
    service = {
        ...
        "healthcheck": {"test": ["CMD", "curl", "-fsS", "http://127.0.0.1:8001/index.php/publicknowledge/login"],
...
def execute_cypress(run, entrypoint, *arguments, display=None, authority=None):
    ...
    # Cypress 5's entrypoint passes the GPU switch directly to Electron.
