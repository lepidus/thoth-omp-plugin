...
The tests use disposable OMP 3.3 and Thoth instances, without changing your local
installation or writing to public Thoth APIs.

## Requirements

Docker Compose v2, Python 3.9+, Linux/amd64 (or compatible emulation), and an OMP
`stable-3_3_0` MySQL dataset for context `publicknowledge`, containing `database.sql`,
...
`run` executes the suite once; add `--spec ThothRegistration.spec.js` for one spec.
...
`ThothRegistration.spec.js` seeds a complete published book in OMP, registers it through
...
OMP 3.3 stores subjects as strings: the fixture uses classification prefixes.
