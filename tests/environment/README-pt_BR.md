...
Os testes usam OMP 3.3 e Thoth descartáveis, sem alterar sua instalação local
...
- Dataset MySQL do OMP `stable-3_3_0`, contexto `publicknowledge`, com
...
```sh
python3 tests/environment/environment.py run --spec ThothRegistration.spec.js --apply
```
...
`ThothRegistration.spec.js` prepara no OMP um livro publicado com metadados completos,
...
website; idioma, assuntos e referências; PDF, EPUB e impresso, ISBN,
...
Neste core, os assuntos são strings; a fixture usa prefixos de classificação.
O campo estruturado de afiliação ROR não está disponível: o teste verifica que nenhuma
afiliação é inventada. Após publicar, o status Thoth é conferido ao recarregar a página.
Para versões simultâneas, informe um `--port` distinto na primeira preparação de cada ambiente.
