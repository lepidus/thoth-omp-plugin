...
Las pruebas usan OMP 3.3 y Thoth desechables, sin modificar la instalación local
ni escribir en las APIs públicas de Thoth.

## Requisitos

Docker Compose v2, Python 3.9+, Linux/amd64 (o emulación compatible) y un dataset
MySQL de OMP `stable-3_3_0`, contexto `publicknowledge`, con `database.sql`, `files/`
...
`run` ejecuta la suite una vez; añada `--spec ThothRegistration.spec.js` para una spec.
...
`ThothRegistration.spec.js` prepara en OMP un libro publicado con metadatos completos,
