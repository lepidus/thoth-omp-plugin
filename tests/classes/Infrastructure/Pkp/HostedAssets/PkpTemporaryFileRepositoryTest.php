...
final class PkpTemporaryFileRepositoryTest extends TestCase
{
    ...
    {
        $temporaryFile = new Pkp33TemporaryFileDouble($this->filePath, 'book.PDF');
        $files = new Pkp33TemporaryFileManagerDouble($temporaryFile);
    ...
    {
        $temporaryFile = new Pkp33TemporaryFileDouble($this->filePath, 'feature.MP4');
        $files = new Pkp33TemporaryFileManagerDouble($temporaryFile);
    ...
    {
        $files = new Pkp33TemporaryFileManagerDouble(null);
...
final class Pkp33TemporaryFileManagerDouble
{
    public array $reads = [];
    public array $deletions = [];
    private ?Pkp33TemporaryFileDouble $file;

    public function __construct(?Pkp33TemporaryFileDouble $file)
    {
        $this->file = $file;
    }

    public function getFile(int $fileId, int $userId): ?Pkp33TemporaryFileDouble
    {
    ...
    public function deleteById(int $fileId, int $userId): void
    {
        $this->deletions[] = [$fileId, $userId];
    }
}

final class Pkp33TemporaryFileDouble
{
    private string $path;
